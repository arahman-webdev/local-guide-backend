...
// -------------------------
 const sslSuccessHandler = async (req: Request, res: Response, next: NextFunction) => {
  console.log("🔵 SSL Success Callback Received:", req.query);
  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await tx.payment.update({
        where: { 
          bookingId: actualBookingId  // Use bookingId which is the unique field
        },
        data: { 
          status: "COMPLETED",
          updatedAt: new Date()
    ...
    console.error("❌ SSL Success Handler Error:", error.message);
